...
function Admin() {
  ...
  const [siteAppearance, setUnidadeAppearance] = useState<PortalUnidadeAppearance | null>(null)
  const [sites, setUnidades] = useState<UnidadeNode[]>([])
  const [allowedUnidades, setAllowedUnidades] = useState<AllowedUnidade[]>([])
  const [selectedUnidadeId, setSelectedUnidadeId] = useState(() => safeStorageGet('admin_selected_site_id', 'ALL'))
  ...
  const [detailUnidadeId, setDetailUnidadeId] = useState(() => decodeURIComponent(window.location.pathname.startsWith('/admin/sites/') ? window.location.pathname.split('/').pop() || '' : ''))
  ...
  const scopedPath = (path: string) => selectedUnidadeId === 'ALL' ? path : `${path}?siteId=${encodeURIComponent(selectedUnidadeId)}`
  const handleUnidadeChange = (siteId: string) => {
    safeStorageSet('admin_selected_site_id', siteId)
    setSelectedUnidadeId(siteId)
  }

  const openUnidadeDetail = (siteId: string) => {
    setDetailUnidadeId(siteId)
    setActiveSection('site-detail')
    window.history.pushState(null, '', `/admin/sites/${encodeURIComponent(siteId)}`)
  }
  const leaveUnidadeDetail = () => {
    setDetailUnidadeId('')
  ...
  const load = async () => {
    ...
    setRefreshing(true)
    try {
      const [me, allowedUnidadeRows, dash, chartRows, maint, appearanceRow, selectedAppearanceRow, siteRows, noticeRows, auditRows, voucherRows, visitorRows, accessPointRows, sessionRows, adminRows, invitationRows, healthRow, attemptRows] = await Promise.all([
        api<AdminMe>('/api/admin/me'),
        api<AllowedUnidade[]>('/api/admin/sites/allowed').catch(() => []),
        ...
        selectedUnidadeId === 'ALL' ? Promise.resolve(null) : api<PortalUnidadeAppearance>(`/api/admin/portal-appearance/site/${encodeURIComponent(selectedUnidadeId)}`).catch(() => null),
        api<UnidadeNode[]>(scopedPath('/api/admin/sites')).catch(() => []),
    ...
    const safeAllowedUnidades = asArray(allowedUnidadeRows)
    setAdmin(normalizedMe)
    setAllowedUnidades(safeAllowedUnidades)
    const canUseAll = normalizedMe.canSelectAllUnidades || normalizedMe.siteIds.length > 1
    if (safeAllowedUnidades.length && selectedUnidadeId !== 'ALL' && !safeAllowedUnidades.some((site) => site.siteId === selectedUnidadeId)) handleUnidadeChange(canUseAll ? 'ALL' : safeAllowedUnidades[0].siteId)
    if (!canUseAll && selectedUnidadeId === 'ALL' && safeAllowedUnidades.length === 1) handleUnidadeChange(safeAllowedUnidades[0].siteId)
    setDashboard(dash)
    setDashboardCharts(chartRows || EMPTY_DASHBOARD_CHARTS)
    setMaintenance(maint)
    setAppearance(appearanceRow)
    setUnidadeAppearance(selectedAppearanceRow)
    setUnidades(asArray(siteRows))
  ...
  useEffect(() => { if (admin) void load().catch(() => undefined) }, [selectedUnidadeId])
  useEffect(() => {
  ...
  const resetUnidadeAppearance = async () => {
    if (selectedUnidadeId === 'ALL') return
    setAppearanceSaving(true)
    setAppearanceMessage('')
    try {
      await api(`/api/admin/portal-appearance/site/${encodeURIComponent(selectedUnidadeId)}`, { method: 'DELETE' })
  ...
  const saveAppearance = async () => {
    ...
    setAppearanceMessage('')
    try {
      if (selectedUnidadeId !== 'ALL' && siteAppearance) {
        const updated = await api<PortalUnidadeAppearance>(`/api/admin/portal-appearance/site/${encodeURIComponent(selectedUnidadeId)}`, { method: 'PUT', body: JSON.stringify(siteAppearance) })
        setUnidadeAppearance(updated)
  ...
  return (
    <main className="admin-app">
      ...
      <section className="admin-main" aria-label="Conteudo administrativo">
        <AdminHeader admin={admin} maintenance={maintenance} refreshing={refreshing} lastUpdatedAt={lastUpdatedAt} refreshError={refreshError} allowedUnidades={allowedUnidades} selectedUnidadeId={selectedUnidadeId} onUnidadeChange={handleUnidadeChange} onRefresh={() => void load().catch(() => undefined)} onLogout={logout} onMenu={() => setMenuOpen(true)} />
        <AdminSectionErrorBoundary section={activeSection}>
          ...
          {activeSection === 'vouchers' ? <VoucherPanel vouchers={vouchers} allowedUnidades={allowedUnidades} selectedUnidadeId={selectedUnidadeId} canCreateGlobal={admin?.role === 'SUPERADMIN'} canManage={admin?.role !== 'VIEWER'} onChanged={load} /> : null}
          {activeSection === 'notices' ? <NoticeAdminPanel notices={notices} allowedUnidades={allowedUnidades} selectedUnidadeId={selectedUnidadeId} canSelectAllUnidades={Boolean(admin?.canSelectAllUnidades)} canManage={admin?.role !== 'VIEWER'} onChanged={load} /> : null}
          {activeSection === 'maintenance' ? maintenance ? <MaintenanceAdminPanel maintenance={maintenance} allowedUnidades={allowedUnidades} selectedUnidadeId={selectedUnidadeId} canManage={admin?.role !== 'VIEWER'} saving={savingMaintenance} feedback={maintenanceMessage} onChange={setMaintenance} onSave={saveMaintenance} /> : <AdminRouteState title="Manutenção" message="Configuração de manutenção ainda não carregada." /> : null}
          {activeSection === 'sites' ? <UnidadesPanel sites={sites} onOpen={openUnidadeDetail} /> : null}
          {activeSection === 'site-detail' ? <UnidadeDetailPanel siteId={detailUnidadeId || selectedUnidadeId} sites={sites} visitors={visitors} sessions={sessions} accessPoints={accessPoints} vouchers={vouchers} notices={notices} maintenance={maintenance} onBack={leaveUnidadeDetail} onOpenSection={(section) => setActiveSection(section)} /> : null}
          {activeSection === 'access-points' ? <AccessPointsPanel accessPoints={accessPoints} /> : null}
          {activeSection === 'admins' ? <AdminsPanel admin={admin} admins={admins} invitations={adminInvitations} allowedUnidades={allowedUnidades} onChanged={load} onToast={notifyAdmin} /> : null}
          {activeSection === 'audit' ? <AuditPanel audit={audit} /> : null}
          {activeSection === 'portal-sites' ? <PortalUnidadesPanel sites={sites} allowedUnidades={allowedUnidades} selectedUnidadeId={selectedUnidadeId} onSelectUnidade={handleUnidadeChange} onOpenPortal={(tab = 'visual') => { setPortalEditorTab(tab); setActiveSection('portal'); window.history.pushState(null, '', '/admin/portal') }} onOpenMaintenance={() => { setActiveSection('maintenance'); window.history.pushState(null, '', '/admin/maintenance') }} onChanged={load} onToast={notifyAdmin} /> : null}
          {activeSection === 'portal' ? appearance ? <SettingsPanel admin={admin} maintenance={maintenance} appearance={selectedUnidadeId !== 'ALL' && siteAppearance ? siteAppearance : appearance} allowedUnidades={allowedUnidades} selectedUnidadeId={selectedUnidadeId} siteAppearance={siteAppearance} notices={notices} activeTab={portalEditorTab} saving={appearanceSaving} feedback={appearanceMessage} onTabChange={setPortalEditorTab} onChange={(value) => selectedUnidadeId !== 'ALL' && siteAppearance ? setUnidadeAppearance({ ...siteAppearance, ...value }) : setAppearance(value)} onSave={saveAppearance} onResetUnidade={resetUnidadeAppearance} /> : <AdminRouteState title="Portal público" message="Configurações do portal ainda não carregadas." /> : null}
...
function DashboardHome({ dashboard, charts, maintenance, sites, notices, vouchers, audit, onSelect }: { dashboard: Dashboard; charts: DashboardChartsData; maintenance: MaintenanceAdmin | null; sites: UnidadeNode[]; notices: AdminNotice[]; vouchers: Voucher[]; audit: AuditEntry[]; onSelect: (section: AdminSection, filter?: SessionFilter) => void }) {
  ...
  return (
    <div className="admin-content">
      ...
      <UnidadeBreakdown sites={sites} notices={notices} vouchers={vouchers} />
      <section className="ops-grid" aria-label="Conteudo operacional">
        <SessionsRecent />
        <UnidadesPanel sites={sites} compact />
...
function DashboardCharts({ dashboard, charts, sites, notices, vouchers, onSelect }: { dashboard: Dashboard; charts: DashboardChartsData; sites: UnidadeNode[]; notices: AdminNotice[]; vouchers: Voucher[]; onSelect: (section: AdminSection, filter?: SessionFilter) => void }) {
    ...
    <section className="dashboard-visual-grid expanded" aria-label="Gráficos operacionais do portal">
      ...
      <Panel title="Capacidade observada" icon={<Wifi />} compact>
        <div className="dashboard-donut-grid">
          ...
          <div className="dashboard-donut-copy"><strong>{totalClients}</strong><span>dispositivos conectados</span><strong>{totalAps}</strong><span>pontos de acesso</span></div>
        </div>
      </Panel>
...
}function UnidadeBreakdown({ sites, notices, vouchers }: { sites: UnidadeNode[]; notices: AdminNotice[]; vouchers: Voucher[] }) {
  ...
  return <section className="site-breakdown-grid" aria-label="Resumo por unidade">{sites.map((site) => {
    ...
    return <article className="site-breakdown-card" key={siteId}><div><strong>{site.name}</strong><span>{site.status}</span></div><div className="site-method-chips">{(site.authMethods?.length ? site.authMethods : (["voucher", "cpf", "email"] as Method[])).map((method) => <span key={method}>{methodLabel(method)}</span>)}</div><dl><div><dt>APs</dt><dd>{site.aps}</dd></div><div><dt>Dispositivos conectados</dt><dd>{site.connectedClients}</dd></div><div><dt>Sessões</dt><dd>{site.sessions}</dd></div><div><dt>Vouchers</dt><dd>{siteVouchers}</dd></div><div><dt>Avisos</dt><dd>{siteNotices}</dd></div></dl></article>
  })}</section>
}
...
function UnidadesPanel({ sites, compact = false, onOpen }: { sites: UnidadeNode[]; compact?: boolean; onOpen?: (siteId: string) => void }) {
...
function PortalUnidadesPanel({ sites, allowedUnidades, selectedUnidadeId, onSelectUnidade, onOpenPortal, onOpenMaintenance, onChanged, onToast }: { sites: UnidadeNode[]; allowedUnidades: AllowedUnidade[]; selectedUnidadeId: string; onSelectUnidade: (siteId: string) => void; onOpenPortal: (tab?: PortalEditorTab) => void; onOpenMaintenance: () => void; onChanged: () => Promise<void>; onToast: (tone: AdminToastTone, title: string, message: string) => void }) {
  const [editingUnidadeId, setEditingUnidadeId] = useState('')
  const [draftMethods, setDraftMethods] = useState<Method[]>([])
  const [busyUnidadeId, setBusyUnidadeId] = useState('')
  const [feedback, setFeedback] = useState('')
  const rows = (allowedUnidades.length ? allowedUnidades : sites.map((site) => ({ siteId: site.siteId || site.name, name: site.name, allowed: true }))).map((allowed) => {
  ...
  const openMethodsEditor = (row: { siteId: string; authMethods: Method[] }) => {
    onSelectUnidade(row.siteId)
    setEditingUnidadeId(row.siteId)
  ...
  const saveMethods = async (row: { siteId: string; name: string }) => {
    setBusyUnidadeId(row.siteId)
    setFeedback('')
    try {
      const current = await api<PortalUnidadeAppearance>(`/api/admin/portal-appearance/site/${encodeURIComponent(row.siteId)}`)
      await api<PortalUnidadeAppearance>(`/api/admin/portal-appearance/site/${encodeURIComponent(row.siteId)}`, { method: 'PUT', body: JSON.stringify({ ...current, siteName: current.siteName || row.name, authMethods: draftMethods }) })
  ...
  return <div className="admin-content"><PageHeader title="Portais por unidade" description="Controle de forma rápida quais métodos aparecem para cada portal público." /><section className="portal-sites-grid">{rows.map((row) => {
    const isSelected = selectedUnidadeId === row.siteId
    const isEditing = editingUnidadeId === row.siteId
    return <article className={`portal-site-card ${isSelected ? 'selected' : ''} ${isEditing ? 'editing' : ''}`} key={row.siteId}>
      <div className="portal-site-card-head"><div><strong>{row.name}</strong><span>{row.site?.status || 'Unidade configurado'}</span></div>{isSelected ? <b>Selecionado</b> : null}</div>
      ...
      <div className="portal-site-actions"><button className="soft-button" type="button" onClick={() => openMethodsEditor(row)}>Editar métodos</button><button className="soft-button" type="button" onClick={() => { onSelectUnidade(row.siteId); onOpenPortal('visual') }}>Visual</button><button className="soft-button" type="button" onClick={() => { onSelectUnidade(row.siteId); onOpenMaintenance() }}>Manutenção</button></div>
      {isEditing ? <div className="portal-site-method-editor" role="group" aria-label={`Editar métodos de ${row.name}`}>
        ...
        <div className="portal-site-editor-actions"><button className="primary" type="button" onClick={() => void saveMethods(row)} disabled={busyUnidadeId === row.siteId}>{busyUnidadeId === row.siteId ? 'Salvando...' : 'Salvar métodos'}</button><button className="soft-button" type="button" onClick={() => setEditingUnidadeId('')} disabled={busyUnidadeId === row.siteId}>Cancelar</button></div>
      </div> : null}
    </article>
...
function SystemHealthPanel({ health, attempts, admin, busy, onRefresh, onTestEmail }: { health: SystemHealth | null; attempts: AuthAttemptRow[]; admin: AdminMe | null; busy: boolean; onRefresh: () => void; onTestEmail: (email: string) => Promise<void> }) {
  ...
  return <div className="admin-content"><PageHeader title="Status do sistema" description="Confira se os principais serviços do portal estão disponíveis." action={<button className="soft-button" type="button" onClick={onRefresh}>Atualizar</button>} />{health ? <section className="health-grid"><Panel title="Dados" icon={<Gauge />} compact><div className="health-stack"><div><span>Banco de dados</span><HealthStatusBadge value={health.database.status} /></div><div><span>Estrutura</span><HealthStatusBadge value={health.schema.status} /></div>{health.schema.findings.length ? <p className="error">Há pendências na estrutura de dados.</p> : <p className="success">Estrutura atualizada.</p>}</div></Panel><Panel title="Rede Wi-Fi" icon={<Radio />} compact><div className="health-stack"><div><span>Status</span><HealthStatusBadge value={health.unifi.status} /></div><div><span>Unidades encontradas</span><strong>{health.unifi.sites}</strong></div>{health.unifi.message ? <p className="error">Não foi possível consultar a rede neste momento.</p> : <p className="success">Controle da rede respondendo normalmente.</p>}</div></Panel><Panel title="E-mail" icon={<FileClock />} compact><div className="health-stack"><div><span>Serviço configurado</span><HealthStatusBadge value={health.smtp.configured} /></div><label htmlFor="health-email-test">Enviar teste para<input id="health-email-test" value={targetEmail} onChange={(event) => setTargetEmail(event.target.value)} inputMode="email" autoComplete="email" /></label><button className="primary admin-save" type="button" onClick={() => void runEmailTest()} disabled={busy || !health.smtp.configured}>{busy ? 'Enviando...' : 'Enviar e-mail de teste'}</button>{message ? <p className={message.includes('falhou') || message.includes('Informe') ? 'error' : 'success'} role="status">{message}</p> : null}</div></Panel><Panel title="Imagens e arquivos" icon={<MonitorCheck />} compact><div className="health-stack"><div><span>Armazenamento</span><HealthStatusBadge value={Boolean(health.media && health.media.status === 'ok')} /></div>{health.media?.message ? <p className="error">O armazenamento de imagens requer atenção.</p> : <p className="success">Envio e exibição de imagens disponíveis.</p>}</div></Panel><Panel title="Proteções" icon={<Settings />} compact><div className="health-stack"><div><span>Verificação adicional no login</span><HealthStatusBadge value={health.runtime.adminEmailMfaRequired} /></div><div><span>Ambiente protegido</span><HealthStatusBadge value={health.runtime.environment === 'production'} /></div></div></Panel></section> : <Panel title="Status indisponível" icon={<AlertTriangle />}><EmptyState message="Ainda não foi possível carregar o status do sistema." /></Panel>}<Panel title="Eventos recentes do portal" icon={<History />} compact><div className="auth-attempt-filters"><label>Método<select value={attemptMethod} onChange={(event) => setAttemptMethod(event.target.value)}><option value="all">Todos</option>{attemptMethods.map((method) => <option key={method} value={method}>{methodLabel(method)}</option>)}</select></label><label>Resultado<select value={attemptResult} onChange={(event) => setAttemptResult(event.target.value as 'all' | 'success' | 'failed')}><option value="all">Todos</option><option value="success">Aceitas</option><option value="failed">Falhas</option></select></label><span>{filteredAttempts.length} evento(s)</span></div>{filteredAttempts.length ? <div className="auth-attempt-list">{filteredAttempts.slice(0, 80).map((row) => <article key={row.id} className={row.success ? 'ok' : 'error'}><div><strong>{methodLabel(row.method)}</strong><span>{formatClock(row.createdAt)}</span></div><p>{row.success ? 'Autenticação aceita' : 'Falha'}{row.reason ? ` · ${row.reason}` : ''}</p></article>)}</div> : <EmptyState message="Nenhum evento encontrado para os filtros atuais." />}</Panel></div>
}
function UnidadeDetailPanel({ siteId, sites, visitors, sessions, accessPoints, vouchers, notices, maintenance, onBack, onOpenSection }: { siteId: string; sites: UnidadeNode[]; visitors: ClientRow[]; sessions: GuestSessionRow[]; accessPoints: AccessPoint[]; vouchers: Voucher[]; notices: AdminNotice[]; maintenance: MaintenanceAdmin | null; onBack: () => void; onOpenSection: (section: AdminSection) => void }) {
...
function VisitorsPanel({ visitors, canManage, busy, onEndSession }: { visitors: ClientRow[]; canManage: boolean; busy: boolean; onEndSession: (sessionId: string) => Promise<void> }) {
  const [query, setQuery] = useState('')
  const [site, setUnidade] = useState('ALL')
  ...
  const filtered = visitors.filter((row) => {
    ...
    const rowUnidade = textValue(row, ['siteName', 'siteId'])
    const rowStatus = boolValue(row, 'authorized') ? 'authorized' : textValue(row, ['portalStatus', 'status']).toLowerCase()
    return (!query || blob.includes(query.toLowerCase())) && (site === 'ALL' || rowUnidade === site) && (status === 'ALL' || rowStatus.includes(status.toLowerCase()))
  })
...
function AdminsPanel({ admin, admins, invitations, allowedUnidades, onChanged, onToast }: { admin: AdminMe | null; admins: AdminUserRow[]; invitations: AdminInviteResponse[]; allowedUnidades: AllowedUnidade[]; onChanged: () => Promise<void>; onToast: (tone: AdminToastTone, title: string, message: string) => void }) {
  const canManage = admin?.role === 'SUPERADMIN'
  const [open, setOpen] = useState(false)
  const [busy, setBusy] = useState(false)
  const [message, setMessage] = useState('')
  const [invite, setInvite] = useState<AdminInviteResponse | null>(null)
  const [form, setForm] = useState({ name: '', email: '', role: 'ADMIN', siteIds: allowedUnidades.map((site) => site.siteId) })
  const [editingAccess, setEditingAccess] = useState<AdminUserRow | null>(null)
  const [accessUnidadeIds, setAccessUnidadeIds] = useState<string[]>([])
  const siteLabel = (siteId: string) => allowedUnidades.find((site) => site.siteId === siteId)?.name || siteId
  const adminUnidadeLabel = (row: AdminUserRow) => row.canSelectAllUnidades ? 'Todos os sites' : row.siteIds?.length ? row.siteIds.map(siteLabel).join(', ') : 'Sem site atribuído'
  ...
  const toggleInviteUnidade = (siteId: string) => {
    setForm((current) => {
  ...
  const toggleAccessUnidade = (siteId: string) => {
    setAccessUnidadeIds((current) => current.includes(siteId) ? current.filter((item) => item !== siteId) : [...current, siteId])
  }
  const openAccessEditor = (row: AdminUserRow) => {
    setEditingAccess(row)
    setAccessUnidadeIds(row.siteIds ?? [])
    setMessage('')
  }
  const saveAccess = async () => {
    ...
    setMessage('')
    try {
      await api(`/api/admin/admins/${encodeURIComponent(editingAccess.id)}/site-access`, { method: 'PUT', body: JSON.stringify({ siteIds: accessUnidadeIds }) })
  ...
  const createInvite = async () => {
    ...
    try {
      ...
      onToast(sent ? 'success' : 'error', sent ? 'Convite enviado' : 'Convite sem e-mail', sent ? `Enviado para ${response.email}.` : 'O SMTP falhou. Use o link manual e confira a tela Saúde.')
      await onChanged()
    } catch (err) {
  ...
  const renewInvite = async (inviteId: string) => {
    ...
    try {
      ...
      setMessage(nextMessage)
      onToast(sent ? 'success' : 'error', sent ? 'Novo link enviado' : 'Novo link sem e-mail', sent ? `Enviado para ${response.email}.` : 'O SMTP falhou. Copie o link manual e confira a tela Saúde.')
      await onChanged()
    } catch (err) {
...
function SettingsPanel({ admin, maintenance, appearance, allowedUnidades, selectedUnidadeId, siteAppearance, notices, activeTab, saving, feedback, onTabChange, onChange, onSave, onResetUnidade }: { admin: AdminMe | null; maintenance: MaintenanceAdmin | null; appearance: PortalAppearance; allowedUnidades: AllowedUnidade[]; selectedUnidadeId: string; siteAppearance: PortalUnidadeAppearance | null; notices: AdminNotice[]; activeTab: PortalEditorTab; saving: boolean; feedback: string; onTabChange: (value: PortalEditorTab) => void; onChange: (value: PortalAppearance) => void; onSave: () => void; onResetUnidade: () => void }) {
  ...
  return (
    <div className="admin-content">
      <PageHeader title="Portal público" description={canManage ? "Edite visual, mensagens, termos, métodos de acesso por unidade e preview da experiência dos visitantes." : "Consulte a configuração e o preview do portal público. Seu perfil é somente leitura."} />
      ...
      <div className="portal-editor-context">
        <strong>{selectedUnidadeId === 'ALL' ? 'Padrão geral do portal' : `Editando unidade: ${scopeLabel}`}</strong>
        <span>{selectedUnidadeId === 'ALL' ? 'Use a base global para o padrão institucional. Para regras como somente voucher em Esdras, selecione a unidade no topo antes de salvar.' : 'Tudo que for salvo aqui vale apenas para esta unidade, incluindo logo, textos e métodos de acesso.'}</span>
      </div>
      ...
      <section className={`settings-layout visual-editor-layout ${activeTab === 'preview' ? 'preview-only' : ''}`}>
        {activeTab !== 'preview' ? <Panel title={`${tabs.find((tab) => tab.id === activeTab)?.label || 'Editor'} · ${scopeLabel}`} icon={<Settings />}>
          <fieldset className="panel-form settings-form readonly-fieldset" disabled={!canManage}>
            {selectedUnidadeId !== 'ALL' ? <p className="scope-note wide">Estas alterações valem somente para {scopeLabel}. Campos não personalizados seguem o padrão geral do portal.</p> : <p className="scope-note wide">Você está editando o padrão visual usado pelas unidades que não possuem personalização própria.</p>}
            {activeTab === 'visual' ? <>
            ...
            {canManage ? <div className="settings-actions wide"><button className="primary admin-save" type="button" onClick={onSave} disabled={saving}>{saving ? 'Salvando...' : selectedUnidadeId === 'ALL' ? 'Salvar visual global' : 'Salvar visual da unidade'}</button>{selectedUnidadeId !== 'ALL' && siteAppearance?.hasOverride ? <button className="soft-button" type="button" onClick={onResetUnidade} disabled={saving}>Voltar ao visual global</button> : null}</div> : null}
        ...
        <Panel title="Prévia do portal" icon={<MonitorCheck />}>
          <div className="preview-toolbar stacked" role="group" aria-label="Contexto da prévia"><span>{selectedUnidadeId === 'ALL' ? 'Visualizando padrão geral' : `Visualizando como ${scopeLabel}`}</span></div>
          ...
          <div className="prepared-grid single preview-security"><InfoTile title="Sessão segura" value="Ativa" detail="As ações administrativas exigem uma sessão autenticada e protegida." icon={<LockKeyhole />} /><InfoTile title="Conta" value={admin?.email ?? 'Autenticada'} detail="Usuário autenticado no painel." icon={<UserRound />} /><InfoTile title="Modo manutenção" value={maintenance?.maintenanceEnabled ? 'Ativo' : 'Inativo'} detail="Estado atual configurado para o portal." icon={<Clock />} /></div>
        </Panel>
      </section>
    </div>
  )
...
function MaintenanceAdminPanel({
  ...
  allowedUnidades: AllowedUnidade[]
  ...
  const selectedUnidade = allowedUnidades.find((site) => site.siteId === selectedUnidadeId)
  const scopeLabel = selectedUnidadeId === 'ALL' ? 'todos os sites' : selectedUnidade?.name || selectedUnidadeId
