...
function SystemHealthPanel({ health, attempts, admin, busy, onRefresh, onTestEmail }: { health: SystemHealth | null; attempts: AuthAttemptRow[]; admin: AdminMe | null; busy: boolean; onRefresh: () => void; onTestEmail: (email: string) => Promise<void> }) {
  ...
  return <div className="admin-content"><PageHeader title="Status do sistema" description="Confira se os principais serviços do portal estão disponíveis." action={<button className="soft-button" type="button" onClick={onRefresh}>Atualizar</button>} />{health ? <section className="health-grid"><Panel title="Banco e schema" icon={<Gauge />} compact><div className="health-stack"><div><span>Banco</span><HealthStatusBadge value={health.database.status} /></div><div><span>Schema</span><HealthStatusBadge value={health.schema.status} /></div><div><span>Revisão Alembic</span><strong>{health.schema.alembicRevision || 'Não informada'}</strong></div>{health.schema.findings.length ? <p className="error">Pendências: {health.schema.findings.join(', ')}</p> : <p className="success">Migrações compatíveis.</p>}</div></Panel><Panel title="UniFi" icon={<Radio />} compact><div className="health-stack"><div><span>Status</span><HealthStatusBadge value={health.unifi.status} /></div><div><span>Sites retornados</span><strong>{health.unifi.sites}</strong></div>{health.unifi.message ? <p className="error">{health.unifi.message}</p> : <p className="success">Controle da rede respondendo normalmente.</p>}</div></Panel><Panel title="SMTP" icon={<FileClock />} compact><div className="health-stack"><div><span>Configuração</span><HealthStatusBadge value={health.smtp.configured} /></div><div><span>Servidor</span><strong>{health.smtp.host || 'Não configurado'}:{health.smtp.port}</strong></div><div><span>Remetente</span><strong>{health.smtp.from || 'Não configurado'}</strong></div><label htmlFor="health-email-test">Enviar teste para<input id="health-email-test" value={targetEmail} onChange={(event) => setTargetEmail(event.target.value)} inputMode="email" autoComplete="email" /></label><button className="primary admin-save" type="button" onClick={() => void runEmailTest()} disabled={busy || !health.smtp.configured}>{busy ? 'Enviando...' : 'Testar e-mail'}</button>{message ? <p className={message.includes('falhou') || message.includes('Informe') ? 'error' : 'success'} role="status">{message}</p> : null}</div></Panel><Panel title="Imagens e arquivos" icon={<MonitorCheck />} compact><div className="health-stack"><div><span>Armazenamento</span><HealthStatusBadge value={Boolean(health.media && health.media.status === 'ok')} /></div><div><span>Acesso aos arquivos</span><strong>{health.media?.publicBaseUrl || health.runtime.mediaPublicBaseUrl}</strong></div>{health.media?.message ? <p className="error">{health.media.message}</p> : <p className="success">Envio e exibição de imagens disponíveis.</p>}</div></Panel><Panel title="Runtime" icon={<Settings />} compact><div className="health-stack"><div><span>Ambiente</span><strong>{health.runtime.environment}</strong></div><div><span>Verificação adicional no login</span><HealthStatusBadge value={health.runtime.adminEmailMfaRequired} /></div><div><span>Arquivos do portal</span><strong>{health.runtime.mediaPublicBaseUrl}</strong></div><div><span>Portal público</span><strong>{health.runtime.publicBaseUrl}</strong></div><div><span>Painel admin</span><strong>{health.runtime.adminBaseUrl}</strong></div></div></Panel></section> : <Panel title="Status indisponível" icon={<AlertTriangle />}><EmptyState message="Ainda não foi possível carregar o status do sistema." /></Panel>}<Panel title="Eventos recentes do portal" icon={<History />} compact><div className="auth-attempt-filters"><label>Método<select value={attemptMethod} onChange={(event) => setAttemptMethod(event.target.value)}><option value="all">Todos</option>{attemptMethods.map((method) => <option key={method} value={method}>{methodLabel(method)}</option>)}</select></label><label>Resultado<select value={attemptResult} onChange={(event) => setAttemptResult(event.target.value as 'all' | 'success' | 'failed')}><option value="all">Todos</option><option value="success">Aceitas</option><option value="failed">Falhas</option></select></label><span>{filteredAttempts.length} evento(s)</span></div>{filteredAttempts.length ? <div className="auth-attempt-list">{filteredAttempts.slice(0, 80).map((row) => <article key={row.id} className={row.success ? 'ok' : 'error'}><div><strong>{methodLabel(row.method)}</strong><span>{formatClock(row.createdAt)}</span></div><p>{row.success ? 'Autenticação aceita' : 'Falha'}{row.reason ? ` · ${row.reason}` : ''}</p></article>)}</div> : <EmptyState message="Nenhum evento encontrado para os filtros atuais." />}</Panel></div>
}
...
function AdminsPanel({ admin, admins, invitations, allowedSites, onChanged, onToast }: { admin: AdminMe | null; admins: AdminUserRow[]; invitations: AdminInviteResponse[]; allowedSites: AllowedSite[]; onChanged: () => Promise<void>; onToast: (tone: AdminToastTone, title: string, message: string) => void }) {
  ...
  const adminSiteLabel = (row: AdminUserRow) => row.canSelectAllSites ? 'Todos os sites' : row.siteIds?.length ? row.siteIds.map(siteLabel).join(', ') : 'Sem unidade atribuída'
      ...
      <section className={`settings-layout visual-editor-layout ${activeTab === 'preview' ? 'preview-only' : ''}`}>
        {activeTab !== 'preview' ? <Panel title={`${tabs.find((tab) => tab.id === activeTab)?.label || 'Editor'} · ${scopeLabel}`} icon={<Settings />}>
          <fieldset className="panel-form settings-form readonly-fieldset" disabled={!canManage}>
            ...
            {canManage ? <div className="settings-actions wide"><button className="primary admin-save" type="button" onClick={onSave} disabled={saving}>{saving ? 'Salvando...' : selectedSiteId === 'ALL' ? 'Salvar visual global' : 'Salvar visual da unidade'}</button>{selectedSiteId !== 'ALL' && siteAppearance?.hasOverride ? <button className="soft-button" type="button" onClick={onResetSite} disabled={saving}>Voltar ao visual global</button> : null}</div> : null}
