...
export type AuthResponse = { sessionId: string; authorized: boolean; authorizedAt: string; expiresAt?: string | null; remainingSeconds: number; totalSeconds: number; sessionMinutes?: number | null; unlimited?: boolean; nextCheckSeconds?: number }
export type EmailCodeResponse = { expiresAt: string }
export type SessionStatus = { sessionId?: string; status: string; authorized: boolean; authorizedAt?: string | null; expiresAt?: string | null; serverNow: string; remainingSeconds: number; remainingMinutes: number; sessionMinutes?: number | null; unlimited?: boolean; totalSeconds: number; warningMessage?: string | null; warningMinutes?: number | null; ssid: string; nextCheckSeconds: number }
...
export type MaintenanceAdmin = { maintenanceEnabled: boolean; maintenanceActive: boolean; maintenanceScheduled: boolean; maintenanceExpired?: boolean; maintenanceStatus?: 'disabled' | 'scheduled' | 'active' | 'expired'; maintenanceNextChangeAt?: string | null; maintenanceScope?: string; maintenanceInherited?: boolean; maintenanceTitle: string; maintenanceMessage: string; maintenanceStartAt?: string | null; maintenanceEndAt?: string | null; maintenanceImageUrl: string; maintenanceVisualConfig?: Record<string, unknown>; updatedAt?: string | null }
export type SystemHealth = { status: string; database: { status: string }; schema: { status: string; findings: string[]; alembicRevision?: string | null }; unifi: { status: string; sites: number; message: string }; media?: { status: string; message: string; publicBaseUrl: string }; smtp: { configured: boolean; host: string; port: number; from: string }; runtime: { environment: string; adminEmailMfaRequired: boolean; mediaPublicBaseUrl: string; publicBaseUrl: string; adminBaseUrl: string } }
...
export type Voucher = { id: string; codeLabel: string; description: string; status: string; durationMinutes: number; unlimitedDuration?: boolean; site: string; siteId?: string; siteName?: string; enabled: boolean; usedCount: number; expiresAt?: string | null; maxDevices: number; dataLimitMb?: number | null; downloadLimit?: number | null; uploadLimit?: number | null; deviceLimit: number; isActive: boolean; revokedAt?: string | null; createdAt: string }
export type CreatedVoucherCode = { id: string; code: string; codeLabel: string; site: string; durationMinutes: number; unlimitedDuration?: boolean; expiresAt?: string | null }
export type VoucherBatchCreateResponse = { created: number; vouchers: CreatedVoucherCode[]; emailDeliveryStatus?: 'not_requested' | 'sent' | 'failed' | string; emailSentTo?: string | null }
