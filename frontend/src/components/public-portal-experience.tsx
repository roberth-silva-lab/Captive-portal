...
function VoucherField(props: PublicPortalExperienceProps & { preview: boolean; method: Method }) {
  ...
  return <label className="field-label voucher-code-field" htmlFor="portal-voucher"><span>Código do voucher</span>{displayValue ? <strong className="voucher-readable-code" aria-hidden="true">{displayValue}</strong> : null}<div className={`voucher-entry ${props.fieldError === 'identifier' ? 'input-error' : ''}`}><Ticket aria-hidden="true" /><input id="portal-voucher" value={value} onChange={(event) => props.onIdentifierChange?.(event.target.value)} inputMode="text" autoCapitalize="characters" autoComplete="one-time-code" autoCorrect="off" spellCheck={false} aria-label="Código do voucher" maxLength={12} placeholder="RF-ABCD-1234" readOnly={props.preview} autoFocus />{value && !props.preview ? <button type="button" aria-label="Limpar voucher" onClick={() => props.onIdentifierChange?.('')}><X /></button> : null}</div><small className="field-help">Digite ou cole o código entregue pela equipe. Conferimos letras e números antes de liberar.</small></label>
}
